...
public:
    ...
    ws_deque()
    {
		...
		alloc_block();
		bottom_.block_id_ = VAR(bottom_.block_)->header_.id_;
		VAR(top_.block_) = VAR(bottom_.block_);
		t.top_block_id_ = static_cast<unsigned short>(VAR(top_.block_).load()->header_.id_);
		t.bottom_block_id_ = static_cast<unsigned short>(VAR(top_.block_).load()->header_.id_);
        VAR(top_.info_) = t;
    }

	~ws_deque()
	{
		for (block* p = VAR(top_.block_), *next; p; p = next)
		{
    ...
    {
        RL_ASSERT(VAR(bottom_.block_)->header_.id_);

        VAR(bottom_.block_)->data_[bottom_.real_index_].store(i, std::memory_order_release);
    ...
    void* pop_unbalanced()
    {
		...
		{
			bottom_.real_index_ -= 1;
            void* i = VAR(bottom_.block_)->data_[bottom_.real_index_].load(std::memory_order_consume);
			return i;
		}
		else
		{
...
retry:
		for (;;)
		{
			...
			if (b->header_.id_ != old.top_block_id_)
			{
                ...
                if (top_.block_.compare_exchange_strong(old_b, b, std::memory_order_seq_cst, std::memory_order_seq_cst))
				{
					block* cur_b = old_b;
					do
					{
			...
			if (top_.info_.compare_exchange_strong(old, mod, std::memory_order_seq_cst, std::memory_order_relaxed))
			{
				if (next_block)
				{
					if (top_.block_.compare_exchange_strong(b, next_block, std::memory_order_seq_cst, std::memory_order_seq_cst))
					{
						pdr_defer(b);
					}
				}
...
private:
	...
	void alloc_block()
	{
		...
		block* b = VAR(bottom_.block_) ? VAR(bottom_.block_)->header_.next_.load(std::memory_order_relaxed) : 0;
		if (0 == b)
		{
			b = new block;
            ...
            b->header_.prev_.store(VAR(bottom_.block_), std::memory_order_relaxed);
			if (VAR(bottom_.block_))
				VAR(bottom_.block_)->header_.next_.store(b, std::memory_order_relaxed);
	...
	void* pop_slow()
	{
		bottom_.block_seq_ -= 1;
		bottom_.block_seq_ &= 0xffff;
        VAR(bottom_.block_) = VAR(bottom_.block_)->header_.prev_.load(std::memory_order_relaxed);
		...
		bottom_.real_block_id_ = VAR(bottom_.block_)->header_.id_;
		bottom_.real_index_ = block::item_count - 1;

        top::info i = top_.info_.load(std::memory_order_relaxed);

        RL_ASSERT(VAR(bottom_.block_)->header_.id_ == bottom_.block_seq_);
        ...
        void* v = VAR(bottom_.block_)->data_[block::item_count - 1].load(std::memory_order_consume);
        return v;
	}
	...
	void check_bottom()
	{
		...
		for (;;)
		{
			...
			*/
            ...
            if (top_.info_.compare_exchange_strong(old, i, std::memory_order_seq_cst, std::memory_order_relaxed))
			{
